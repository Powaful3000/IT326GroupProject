...
DROP TABLE IF EXISTS bookmarked_posts;    -- Junction table for user bookmarks
DROP TABLE IF EXISTS blocked_users;        -- Junction table for blocked users
DROP TABLE IF EXISTS friends;              -- Junction table for friendships
DROP TABLE IF EXISTS friend_requests;      -- Table for pending friend requests
DROP TABLE IF EXISTS post_groups;          -- Junction table linking posts to groups
DROP TABLE IF EXISTS student_tags;         -- Junction table linking students to tags
DROP TABLE IF EXISTS group_memberships;    -- Junction table for group members

-- Then drop dependent tables
DROP TABLE IF EXISTS tags;                 -- Tags table
DROP TABLE IF EXISTS posts;                -- Posts table
DROP TABLE IF EXISTS student_groups;       -- Groups table
DROP TABLE IF EXISTS students;             -- Core students table
...
CREATE TABLE posts (
    ...
    FOREIGN KEY (postOwner) REFERENCES students (userID) ON DELETE CASCADE -- Delete posts when student is deleted
);
...
CREATE TABLE friend_requests (
    ...
    FOREIGN KEY (fromUserID) REFERENCES students (userID) ON DELETE CASCADE,
    FOREIGN KEY (toUserID) REFERENCES students (userID) ON DELETE CASCADE
);
